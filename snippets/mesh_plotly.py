...
_names = list(store.field.keys())
field_name = field_name if "field_name" in dir() and field_name else next(
    (n for n in ("h", "height", "q1") if n in _names), _names[0])
time_step = (int(time_step) if ("time_step" in dir() and time_step is not None)
