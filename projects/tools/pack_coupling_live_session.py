"""Build gui/projects/coupling-live-session.zip — the SME<->VOF coupled case as a
...
ZIP = os.path.join(ROOT, "library/zoomy_gui/projects/coupling-live-session.zip")
...
TITLE = "SME-VOF coupling (live)"
sess = {
    "id": "session-coupling-live",
    "title": TITLE,
    "description": ("LIVE two-way SME(1)-VOF preCICE run — needs a foam backend "
                    "(zoomy_openfoam container). ~1 min incl. on-demand compile."),
...
meta = {"version": "1.1", "sessions": [sess], "activeSession": "session-coupling-live"}
